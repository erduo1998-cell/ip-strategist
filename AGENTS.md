...
- 普通任务先确定唯一主路由，只加载 `SKILL.md` 与一个 `references/task-*.md`；不得恢复默认通读 `references/00-11`，不得要求另一任务胶囊作必读前置。
- `SKILL.md` 不超过 12,000 bytes、原则上不超过 180 行；每个任务胶囊不超过 16,000 bytes；`SKILL.md + 单胶囊` 不超过 28,000 bytes。
