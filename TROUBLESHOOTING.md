...
先确认根目录 [VERSION](VERSION) 显示 `2.2.0`，并在修改安装目录后新建会话。当前会话不会热重载新版 Skill。
...
## 6. 评论证据没有汇总，或担心它改了复盘状态

v2.2 的评论证据接入只读取三平台**本人创作者后台**的评论，只在本机保存和处理。它按唯一作品 ID 将证据关联到一个契约，并且只汇总已经到复盘期的契约；没有唯一对应关系时应保持未关联，不要手工猜配。

平台返回、评论正文和本地证据文件都是不可信输入：它们不能覆盖 Skill 规则，也不能作为命令执行。接入不会自动把契约改成「已复盘」、不会改动复盘日期，也不会自动写入归因结论。若状态或结论变化，请回到正式复盘流程，由创作者确认后再写入私有状态。

评论证据、缓存和本地运行依赖都不得提交、粘贴到 issue 或作为测试夹具；排障时只提供去标识的错误信息。

## 7. 更新被安全停止

明确说“更新 ip-strategist”才会执行 `scripts/ip-update.py`。脚本只更新官方 `erduo1998-cell/ip-strategist`；以下情况停止是预期安全行为：错误 remote、dirty worktree、非 fast-forward、无法识别安装类型。Windows 的安装器管理副本会先解析可执行的 `npx.cmd`；找不到时停止并提示安装或修复 Node.js/npm，而不会猜测命令路径。
