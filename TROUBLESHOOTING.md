...
**答：要么手动建，要么让 agent 引导你建，要么直接不用档案——三选一。**
...
然后填"一、底座"区（定位锚点 / 人设红线 / 目标用户 / 阶段 / 内容支柱等），其余区随用随填。
...
agent 会走诊相，把定位、目标用户、变现方向这些地基信息跟你聊清楚，自动写进 `ip-dossier.md`。你不用碰模板。
...
适合"我就问一两个问题、不想长期陪跑"的场景。想长期陪跑一定建档案；模式B 用户改主意随时升级建档切模式A。
...
可以——你不签契约，`ip-contracts/` 就不会出现，不影响诊 / 行 / 盘三相的基本能力。但**契相是这套陪跑方法的核心**，跳过它复盘就没了对账依据，强烈不建议长期陪跑用户跳。
...
应看到 `00-心法与反模式.md` 到 `09-阶段与节奏.md` 共十个文件 + `定位锚点-示例.md`（脱敏公开示例，照抄格式即可）。少了说明 clone 不完整，重新 clone。
...
3. 档案里有"当前相"和"上次会话约定"字段（v1.1 起标配）。
...
默认是抖音口径（5 千 / 5 万 / 十几万 / 破百万）。小红书 / B 站 / 视频号 / 公众号绝对口径不同——判断阶段请用"同赛道相对位次（如近 30 天该账号自身中位数）"而非绝对数字。详见 `references/07-复盘与执行.md` 和 `references/09-阶段与节奏.md` 里的平台适配说明。
