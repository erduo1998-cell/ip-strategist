...
const targetDir = path.join(os.homedir(), '.opencli', 'clis', 'xiaohongshu');
const files = [
...
const installed = [];
for (const [sourceName, targetName] of files) installed.push(await installFile(sourceName, targetName));
...
process.stdout.write('下一步：同一 OpenCLI Profile 需要分别登录 creator.xiaohongshu.com 与 www.xiaohongshu.com。\n');
